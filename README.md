...
GitHub Actions waits for the merged commit’s matching Vercel Production deployment, validates its generated URL as HTTPS on a `vercel.app` hostname, and then runs five standard mobile Lighthouse measurements against [www.doctorderek.com](https://www.doctorderek.com/). Every completed run must remain on that canonical Production origin. The badges and linked HTML report publish the run with the median Performance score.
