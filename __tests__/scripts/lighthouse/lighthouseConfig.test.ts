...
describe("Lighthouse configuration", () => {
  ...
  it("configures three authenticated Preview runs without hardcoding credentials", () => {
    process.env.LIGHTHOUSE_TARGET_URL = "https://preview.example.com"
    process.env.LIGHTHOUSE_NUMBER_OF_RUNS = "3"
    process.env.LIGHTHOUSE_OUTPUT_DIRECTORY = "./preview-results"
    process.env.LIGHTHOUSE_VERCEL_TRUSTED_OIDC_TOKEN = "short-lived-token"

    const configuration = loadConfiguration()

    expect(configuration.ci.collect.url).toEqual([
    ...
    expect(configuration.ci.upload.outputDir).toBe("./preview-results")
  })
