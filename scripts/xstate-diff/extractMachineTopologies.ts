...
const getPropertyName = (propertyName: ts.PropertyName) => {
  if (
...
const getStaticText = (expression: ts.Expression): string | undefined => {
  if (
...
const getMachineVariableName = (callExpression: ts.CallExpression) => {
...
const isMachineFactoryCall = (callExpression: ts.CallExpression) => {
...
const readGuardName = (
  ...
  if (ts.isObjectLiteralExpression(guardExpression)) {
    const guardType = readStaticPropertyText(guardExpression, "type")
...
const readTargetDescriptors = (
  ...
  if (ts.isArrayLiteralExpression(unwrappedExpression)) {
    const targets = unwrappedExpression.elements.map((element) =>
